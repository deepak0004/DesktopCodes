...
int main()
{
     ...
     res = prqu.size()+1;
     while( ( paa.f>=0 ) && ( prqu.size()>0 ) )
     {
         pair< ll, ll > temp = prqu.top();
         if( ( paa.f>=0 ) && ( paa.f >= (temp.s-temp.f+1) ) )
         {
             paa.f -= ( temp.s-temp.f+1 );
             prqu.pop();
             while( ( i<vc.size() ) && ( paa.f<vc[i].f ) )
             {
                 prqu.push( vc[i++] );
             }
         }

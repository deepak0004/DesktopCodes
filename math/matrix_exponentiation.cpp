...
ll arr[110], mat[110][110], mat2[110][110], N, mat4[110][110];
void multip( ll K )
{
    if( K==1 )
        return;
    ...
    rep(i, 1, N)
    {
        rep(j, 1, N)
        {
            rep(k, 1, N)
            {
                mat4[i][j] = ( mat4[i][j] + ( ( mat[i][k] * mat[k][j] ) % MOD ) ) %MOD;
            }
        }
    }
    rep(i, 1, N)
    ...
    if( K%2 )
    {
        rep(i, 1, N)
        {
            rep(j, 1, N)
            {
                rep(k, 1, N)
                {
                    mat4[i][j] = ( mat4[i][j] + ( ( mat[i][k] * mat2[k][j] ) % MOD ) ) %MOD;
                }
            }
        }
        rep(i, 1, N)

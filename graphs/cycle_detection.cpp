...
ll dp[1010][1010], ans, res, N, M;
bool vis[1010][1010];
string st[1010], nextt = "DIMA";
bool ok( ll x, ll y )
{
    if( x<=0 || y<=0 || x>N || y>M )
        return 0;
...
ll dfs( ll x, ll y, ll ind )
{
    ...
    // Cycle is only when vertex in stack are repeated again
    vis[x][y] = 1;  // For cycle checking

    ll i, j;
    ind = (ind+1)%4;
    if( ok(x+1, y) && st[x+1][y]==nextt[ind] )
    {
       ll o1 = dfs(x+1, y, ind);
       res = max(res, o1);
    }
    ...
    if( ok(x, y+1) && st[x][y+1]==nextt[ind] )
    {
       ll o1 = dfs(x, y+1, ind);
       res = max(res, o1);
    }
    ...
    vis[x][y] = 0;   // Remove it from visited stack, so that new checking does not coincide with it

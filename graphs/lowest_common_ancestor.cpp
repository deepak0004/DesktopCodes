...
int depth[100010], parent[100010][20], ans = 1;
...
{
    int lca_ = lca(o1, o2);
    return (depth[o1] + depth[o2] - (2*depth[lca_]-1));
}
void func( int S, int F, int T )
{
    int dist_sf = dist(S, F);
    int dist_tf = dist(T, F);
    int dist_sub = dist(S, T) - 1;

    int op = ( dist_sf + dist_tf - dist_sub ) / 2;

    //trace1(lca_st)
    //trace6(S, F, T, dist_sf, dist_tf, dist_sub)
    ans = max(ans, op);
}
int main()
{

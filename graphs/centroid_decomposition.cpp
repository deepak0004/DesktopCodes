...
ll dp[300010], nodesub[300010], par[300010];
bool vis[300010];
pair<ll, ll> hi[300010];
vector< ll > adj[300010];
void centroid( ll src )
{
    ll i, j;
    vis[ src ] = 1;
    hi[src] = mp(0, 0);
    ...
    rep(i, 0, (int)adj[src].size()-1)
    {
        ...
        hi[src] = max( hi[src], mp( nodesub[ adj[src][i] ], adj[src][i] ) );
        nodesub[ src ] += nodesub[ adj[src][i] ];
    }

    if( (2*hi[src].f) <= nodesub[src] )
    {
        dp[src] = src;
    }
    else
    {
        ll checker_node = dp[ hi[src].s ];
        while( ( max( nodesub[src]-nodesub[checker_node], hi[checker_node].f )*2 ) > nodesub[src] )
        {
           checker_node = par[checker_node];
        }

...
bool flag;
string wordd;
void func( ll ro, ll co )
{
    stack< pair< pair<ll, ll>, ll > > stac;

    ll i, j;
    while( !stac.empty() )
    {
        pair<ll, ll> poss = stac.top().f;
        ll word_pos = stac.top().s;

        rep(i, 0, 7)
        {
            ll newro = poss.f + dx[i];
            ll newco = poss.s + dy[i];

            if( check(newro, newco) )
            {

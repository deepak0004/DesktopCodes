...
ll pre[40], in[40], ind, ans, arr[40], N;
vector<ll> vc[40];
bool vis[40];

ll searchh( ll fi, ll se, ll ele )
{
   ll i;
...
ll cons_tree( ll star, ll endd )
{
    ...
    ll pos = searchh( star, endd, op ), le, ri;
    //trace5(op, pos, ind, star, endd)
    if( pre[ind]>op )
    {
       ri = cons_tree(pos+1, endd);
       le = cons_tree(star, pos-1);
    }
    else
    {
       le = cons_tree(star, pos-1);
...
void dfs( ll ind )
{
    ...
    ans *= max(1, vc[ind].size());
    //trace2(ind, vc[ind].size())

...
int main()
{
    ...
    while(T--)
    {
        ...
        rep(i, 1, (int)st1.length()-1)
        {
            dp[i][0] = st1[i]-'0' + dp[i-1][0];
        }
        rep(i, 1, (int)st2.length()-1)
        {
            dp[0][i] = st2[i]-'0' + dp[0][i-1];
        }
        rep(i, 1, (int)st1.length()-1)
        {
            rep(j, 1, (int)st2.length()-1)
            {
                if( st1[i]==st2[j] )
                {
                    dp[i][j] = dp[i-1][j-1];
                ...
                {
                    dp[i][j] = min( dp[i][j-1] + st2[j]-'0', dp[i-1][j] + st1[i]-'0' );
                }
            }
        }

        cout << dp[(int)st1.length()-1][(int)st2.length()-1] << endl;
    }

...
ll arr[4][1000010], ans, N;
vector<ll> vc;
void max_area( ll row )
{
    ll i, j, element;
    vector<ll> vc;
    stack<ll> stac;
    ...
    rep(i, 0, (int)vc.size()-1)
    {
        while(stac.size())
        {
           if( vc[stac.top()]>vc[i] )
           {
               ...
               if( !stac.size() )
               {
                  // No elements in stack, so vc[element] is smallest bar till i and would be included in whole of i.
                  ans = max(ans, i*vc[element]);
               }
               else
               {
                  // For vc[element] as height of bar, find max area of histogram. This would be from stac.top() ( after deleting vc[element] to i.
                  ans = max( ans, (i-stac.top()-1)*vc[element] );
               }
           }
           else
           {
    ...
    while(stac.size())
    {
        ...
        if( !stac.size() )
        {
            ans = max(ans, N*vc[element]);
        }
        else
        {
            ans = max(ans, (N-stac.top()-1)*vc[element]);
        }
    }
}
int main()

...
{
    //cout << ver << endl;
    ll i, j, o1 = INT_MAX, o2 = INT_MAX;

    vis[ver] = 1;

    rep(i, 0, (int)vc[ver].size()-1)
    {
    	//trace3(o1, o2, ver)
        if( !vis[vc[ver][i]] && o1==INT_MAX )
        {
            o1 = dfs( vc[ver][i] );
        }
        else if( !vis[vc[ver][i]] && o2==INT_MAX )
        {
            o2 = dfs( vc[ver][i] );
        }
    }
    ...
    {
        ans = max(ans, o1+o2+arr[ver]);
        return ( max(o1, o2) + arr[ver] );
    }
    else
    {
        ans = max(ans, arr[ver]);

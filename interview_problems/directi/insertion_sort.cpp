...
ll arr[10010], qtree[40010];
void update(int idx, int start, int en, int id, int x)
{
    ...
    {
        int m = mid(start, en);
        update( 2*idx, start, m, id, x);
        update( 2*idx+1, m+1, en, id, x);
        qtree[idx] =  qtree[2*idx] + qtree[2*idx+1];
    }
}
int query(int idx,int start, int en, int b, int e)
{
    if(b > en || e < start)
    {
        return 0;   // Return is 0 as it won't affect the sum. In maxtree return is INT_MIN, in mintree return is INT_MAX : Therefore these also don't affect the respective trees.
    }
    ...
    {
        int m = mid(start, en);
        int q1 = query( 2*idx, start, m, b, e);
        int q2 = query( 2*idx + 1, m + 1, en, b, e);
        return (q1+q2);
    }
}
int main()
{
    ...
    while(T--)
    {
        ...
        update(1, 0, 10000, arr[0], 1);
        rep(i, 1, N-1)
        {
            ll val = query(1, 0, 10000, arr[i]+1, 10000);
            compp += val;
            if( val!=i )
                compp++;
            swapp += val;

            update(1, 0, 10000, arr[i], 1);
            //trace2(compp, swapp)
        }

        cout << ( compp-swapp ) << endl;
    }

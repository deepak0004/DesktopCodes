...
ll A[4][1010], dp[4][1010], N, K, P;
ll func( ll ind, bool stair )
{
    if( dp[stair][ind]!=-1 )
    {
        return dp[stair][ind];
    ...
    rep(i, ind+1, min(N, ind+K))
    {
       ll op = func(i, stair) + A[stair][ind];
       ll op2 = func(i, !stair) + P + A[stair][ind];

       ans = min(ans, min(op, op2));
    }
...
int main()
{
    ...
    while(T--)
    {
        ...
        cout << min( func(1, 1), func(1, 0) ) << endl;
    }

...
bool dist( ll f1, ll f2, ll s1, ll s2 )
{
    double opop = ( (f1-s1) * (f1-s1) ) + ( (f2-s2) * (f2-s2) );
    opop = sqrt(opop);
    //trace1(opop)
    return ( (opop-50.0) <= epsilon );
}
int main()
{
    ...
    {
       cin >> P >> X >> Y;
       double slope = (double)3.6*P;
       double angle1 = atan2(50, 0);
       double angle2 = atan2(Y-50, X-50);
       double result = ( (angle1-angle2) * 180 ) / PI;
       if (result<0)
       {
            result += 360;
       }
       ...
       cout << "Case #" << ppll << ": ";
       if( ( slope>=result ) && dist(50, 50, X, Y) && P )
       {
           cout << "black" << endl;
       }
       else

...
ll gra[510][510][26], dp[510][26], N, time2[510];
bool vertmark[510];
ll dist[510];
ll minndist()
{
...
void dijk( ll timee )
{
   ...
   rep(i, 1, N-1)
   {
       ...
       rep(j, 1, N)
       {
           //trace2( gra[v1][j][ time2[v1] ] + dist[v1], dist[j] )
           if( !vertmark[j] && dist[v1]!=1e8 && ( dist[j]>( gra[v1][j][ time2[v1] ] + dist[v1] ) )  )
           {
             dist[j] = ( gra[v1][j][ time2[v1] ] + dist[v1] );
             time2[j] = (gra[v1][j][ time2[v1] ] + time2[v1]) % 24;
             if( !time2[j] )
                time2[j] = 24;
           }
       }
...
int main()
{
    ...
    ll i, j, T, M, K, U, V, ppll, timee, k, ti, D, S;
    string st;
    ...
    rep(ppll, 1, T)
    {
       ...
       rep(i, 1, M)
       {
          ...
          {
              cin >> timee;
              gra[U][V][j] = min(gra[U][V][j], timee);
              gra[V][U][j] = min(gra[V][U][j], timee);
          }
       }
       rep(ti, 1, 24)
       {

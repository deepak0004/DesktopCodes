...
double ans, P, Q;
char st[22][22];
void dfs( ll Rs, ll Cs, ll R, ll C, ll S, double ans2, map< pair<ll, ll>, ll > vis, bool flag )
{
    ...
    if( st[Rs][Cs]=='.' && flag )
    {
      ans2 += pow_2(1-Q, vis[mp(Rs, Cs)]) * Q;
      vis[mp(Rs, Cs)]++;
    }
    else if( flag )
    {
      ans2 += pow_2(1-P, vis[mp(Rs, Cs)]) * P;
      vis[mp(Rs, Cs)]++;
    }
    ...
    dfs(Rs+1, Cs, R, C, opop, ans2, vis, flag);
    dfs(Rs-1, Cs, R, C, opop, ans2, vis, flag);
    dfs(Rs, Cs+1, R, C, opop, ans2, vis, flag);
    dfs(Rs, Cs-1, R, C, opop, ans2, vis, flag);
}
int main()
{
    ...
    rep(ppll, 1, T)
    {
       ...
       dfs(Rs+1, Cs+1, R, C, S+1, 0.0, vis, 0);

       cout << "Case #" << ppll << ": " << fixed << setprecision(12) << ans << endl;
    }

...
ll ch[1500];
ll dp[740][740][4][4];
ll func( ll jamie, ll cameroon, bool last_finisher, bool first_started )
{
    ...
    if( (jamie+cameroon)==1440 )
    {
        if( first_started==last_finisher )
        {
            op1 = 0;
        }
        else
    ...
    if( dp[jamie][cameroon][last_finisher][first_started]!=-1 )
    {
        return dp[jamie][cameroon][last_finisher][first_started];
    }

    if( !last_finisher )
    {
        if( ch[jamie+cameroon]!=1 )
        {
          op1 = func(jamie+1, cameroon, last_finisher, first_started);
        }
        if( ch[jamie+cameroon]!=0 )
        {
          op2 = func(jamie, cameroon+1, !last_finisher, first_started) + 1;
        }
    }
    else
    {
        if( ch[jamie+cameroon]!=1 )
        {
          op1 = func(jamie+1, cameroon, !last_finisher, first_started) + 1;
        }
        if( ch[jamie+cameroon]!=0 )
        {
          op2 = func(jamie, cameroon+1, last_finisher, first_started);
        }
    }
    dp[jamie][cameroon][last_finisher][first_started] = min(op1, op2);

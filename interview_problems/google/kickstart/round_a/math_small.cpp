...
int main()
{
    ...
    rep(i, 1, T)
    {
       ...
       ll minn = min(R, C), maxx = max(R, C);

       minn--;
       maxx--;

       ll opop = ( ( ( minn * (minn+1) ) %MOD ) * ( (3*maxx) - minn + 1 ) ) %MOD;
       opop = ( opop * pow_mod(6, MOD-2) ) %MOD;
       ...
       rep(j, 1, min(R-1, C-1))
       {
           ...
           ll opop2 = ( ( ( R * C ) %MOD ) * yy ) %MOD;

           ll opop3 = ( ( ( R * yy ) %MOD ) * j ) %MOD;  //

           ll opop4 = ( ( ( C * yy ) %MOD ) * j ) %MOD;  //

           ll opop5 = ( ( ( R * yy ) %MOD ) * (yy+1) ) %MOD;
           opop5 = ( opop5 * pow_mod(2, MOD-2) ) %MOD;  //

           ll opop6 = ( ( ( C * yy ) %MOD ) * (yy+1) ) %MOD;
           opop6 = ( opop6 * pow_mod(2, MOD-2) ) %MOD;  //

           ll opop7 = ( ( ( j * j ) %MOD ) * yy ) %MOD;          ///// n^2

           ll opop8 = ( ( ( yy * (yy+1) ) %MOD ) * (2*yy + 1) ) %MOD;
           opop8 = ( opop8 * pow_mod(6, MOD-2) ) %MOD;             ///// m^2

           ll opop9 = ( ( ( yy * (yy+1) ) %MOD ) * j ) %MOD;   ///// 2mn


           anss = ( anss + opop2 - opop3 + MOD ) %MOD;
           anss = ( anss - opop4 + MOD ) %MOD;
           anss = ( anss + MOD - opop5 ) %MOD;
           anss = ( anss + MOD - opop6 ) %MOD;
           anss = ( anss + opop7 + opop8 + opop9 ) %MOD;
       }

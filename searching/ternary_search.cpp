...
// Never use int or long array in function, always go for bool( as it is not global memory ).
// Sometimes 5*10^5 passes on hackerrank.
/*
The ternary search function should be first strictly increasing and then strictly decreasing or vice versa : increasing/decreasing --> strictly increasing/decreasing.
Otherwise it won't work if f(mid) becomes equal to f(mid+1).
...
For double ternary search, there can be more than 1 plateau( f(i)==f(i+1) ) still it finds the extrema, however function should be unimodal( increasing becomes equal
extrema and then minimum )
*/
